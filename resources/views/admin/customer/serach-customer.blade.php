                  ...
                  <tbody>
                    @foreach($all_records as $record)
                        ...
                        <td><a href="{{ url('admin/single-customer/detail',$record->id ) }}">{{ $record->service_id }}</a></td>
                            ...
                            <td>@if($record->site_survey_record){{ Carbon\Carbon::parse($record->site_survey_record->date_site_survey)->format('m/d/Y') }}@endif</td>
                            <td>@if($record->landlord_record){{ Carbon\Carbon::parse($record->landlord_record->date_submit_for_landlord)->format('m/d/Y') }}@endif</td>
                            <td>@if($record->landlord_record){{ Carbon\Carbon::parse($record->landlord_record->date_landlord_approval)->format('m/d/Y') }}@endif</td>
                            <td>@if($record->build_record->planned_build_completion_date) {{ Carbon\Carbon::parse($record->build_record->planned_build_completion_date)->format('m/d/Y') }} @endif</td>
