...
class O2capReportExport implements FromCollection,WithHeadings,WithColumnFormatting,ShouldAutoSize
{
    /**
 ...
 public function columnFormats(): array {
        return [
            ...
            'CD' => NumberFormat::FORMAT_DATE_DDMMYYYY, 
            'CE' => NumberFormat::FORMAT_DATE_DDMMYYYY, 
            'CF' => NumberFormat::FORMAT_NUMBER_00,
            'CK' => NumberFormat::FORMAT_DATE_DDMMYYYY,
            'CM' => NumberFormat::FORMAT_DATE_DDMMYYYY, 
            'ED' => NumberFormat::FORMAT_DATE_DDMMYYYY,         
        ];
    }
    ...
    public function get_all_report_list($allResults){
        ...
        foreach($allResults as $key => $result){
            ...
            } elseif(!empty($result['date_po_order_rx']) && empty($result['date_new'])){
                $date_po_order_rx = Carbon::parse($result['date_po_order_rx']);
                $sales_ageing_diff = $new_current_date->diffInDays($date_po_order_rx);
                $sales_ageing = (int)$sales_ageing_diff+1;
            }
            ...
            //     $equal_date =  $cts_date->equalTo($received_cts_date);
            // }
             //for cts ageing
            $cts_ageing = "";
            if(!empty($result['cts_date']) && !empty($result['received_cts_date'])){
           ...
           if($project_duration == 0){
            ...
            //Call BuildMasterFileController calculate_est_complition_date 
            ...
            //Planned Build Completion Date
            $planned_build_completion_date = "";
            if(empty($result['planning_record']['revised_planned_wp2_date'])){
                $planned_wp2_released_date_parse = Carbon::parse($planned_wp2_released_date);
                $revised_planned_wp2_date_total_days = 5+$build_days;
                $planned_build_completion_date = $planned_wp2_released_date_parse->addDays($revised_planned_wp2_date_total_days)->format('Y/m/d');
            } else {
                $planned_wp2_released_date_parse = Carbon::parse($result['planning_record']['revised_planned_wp2_date']);
                $revised_planned_wp2_date_total_days = 5+$build_days;
                $planned_build_completion_date = $planned_wp2_released_date_parse->addDays($revised_planned_wp2_date_total_days)->format('Y/m/d');
            }
           ...
           // if(isset($result['special_build_nrc'])){
            ...
            if(isset($result['thrd_party_nrc'])){
              ...
              if(isset($result['thrd_party_mrc'])){
                  ...
                  //$r_thrd_party_mrc = floatval(str_replace("R","",$thrd_party_mrc));
                  $new_thrd_party_mrc = str_replace("R","",$thrd_party_mrc);
              }

             //check site survey status
             $site_survey_status = 'A) New Sales';
             if(isset($result['site_survey_record']['site_survey_status'])){
               $site_survey_status = $result['site_survey_record']['site_survey_status'];
             } 

             // check landlord status
             $landlord_approval_status = 'A) New Sales';
             if(isset($result['landlord_record']['landlord_approval_status'])){
               $landlord_approval_status = $result['landlord_record']['landlord_approval_status'];
             }

            // check landlord status
            $wayleaves_status = 'A) New Sales';
            if(isset($result['permission_record']['wayleaves_status'])){
            ...
            $rx_in_planning_date = "";
            if(isset($result['planning_record']['rx_in_planning'])){
                $rx_in_planning_date = Carbon::parse($result['planning_record']['rx_in_planning'])->format('Y/m/d');
            }
           ...
           if($show_me_the_money){
			...
			if($rx_in_planning_date){
			   $rx_in_planning_date = Date::dateTimeToExcel(Carbon::parse($rx_in_planning_date));
			}
            ...
            $all_report_lists[] = [
                                    ...
                                    'Landlord Approval Status' => $landlord_approval_status,
                                    'WAYLEAVE STATUS' => $wayleaves_status,
                                    ...
                                    '3rd Party NRC' => floatval($new_thrd_party_nrc),
                                    '3rd Party MRC' => floatval($new_thrd_party_mrc),
                                    ...
                                    'Rx IN PLANNING' => $rx_in_planning_date ?? '',
                                    ...
                                    'Planned Build Date' => $planned_build_date ?? '',
                                    ...
                                    'OSP ASB - TRENCH' => $result['build_record']['actual_osp_build_distance_trench'] ?? '',
                                    'OSP ASB - 3RD PARTY DUCTS' => $result['build_record']['actual_osp_build_distance_3rd_party_ducts'] ?? '',
                                    'OSP ASB - LA EXISTING DUCT' => $result['build_record']['actual_osp_build_la_existing_duct'] ?? '',
                                    'OSP ASB - EXISTING NETWORK' => $result['build_record']['actual_osp_build_la_existing_network'] ?? '',
                                    'OSP ASB - DISTANCE – FOCUS' => $result['build_record']['actual_osp_build_distance_focus'] ?? '',
                                    'OSP ASB – IN BUILDING CONDUITS' => $result['build_record']['actual_osp_build_in_building_conduits'] ?? '',
