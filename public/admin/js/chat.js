$(function () {
    ...
    let pusher = new Pusher('fd2d9d84a885929af982', { 
        cluster: 'ap2',
        forceTLS: true
    }); 
    ...
    $(document).ready(function() {
        $(".first_active")[0].click();
    });

    $('body').on('click', '.chat-toggle', function(e) {
       e.preventDefault();
       
       //hide default first chat
       $('.default_first_chat_list').hide();

       $('.chat-toggle').removeClass('first_active');
...
function loadLatestMessages(container, user_id)
{
    ...
    $.ajax({
        url: base_url2 + "/load-latest-messages",
...
function fetchOldMessages(to_user, old_message_id)
{
    ...
    $.ajax({
        url: base_url2 + "/fetch-old-messages",
...
function getMessageSenderHtml(message)
{
    return `
        ...
        <div class="col-md-2 col-xs-2 avatar">
            <img src="` + base_url2 +  '/public/uploads/users/' +`${message.fromUserProfile}" width="50" height="50" class="img-responsive">
        </div>
    </div>
    `;
}
...
function getMessageReceiverHtml(message)
{
    return `
           ...
           <div class="col-md-2 col-xs-2 avatar">
             <img src="` + base_url2 +  '/public/uploads/users/' +`${message.toUserProfile}" width="50" height="50" class="img-responsive">
           </div>
        <div class="col-md-10 col-xs-10">
